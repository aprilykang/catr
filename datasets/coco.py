...
class CocoCaption(Dataset):
    def __init__(self, root, ann, max_length, limit, transform=train_transform, mode='training'):
        ...
        self.annot = [(self._process(val['id']), val['caption_ko'])
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'validation':
            image = Image.open(os.path.join(self.root, 'COCO_val2014_'+image_id))
        
        elif self.mode == 'training':
            image = Image.open(os.path.join(self.root, 'COCO_train2014_'+image_id))
...
def build_dataset(config, mode='training'):
    if mode == 'training':
        train_dir = os.path.join(config.dir, 'train2014')
        train_file = os.path.join(
            config.dir, 'annotations', 'captions_train2014_korean.json')
        ...
        val_dir = os.path.join(config.dir, 'val2014')
        val_file = os.path.join(
            config.dir, 'annotations', 'captions_val2014_korean.json')
